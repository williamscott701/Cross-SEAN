...
import batch_utils
import utils
from model import LSTMEncoder, Classifier, SequenceCriteria, \
...
def retAddnBatch(addn_data, b_sz, sofar):
...
#             self.embedder.load_state_dict(dict_['state_dict_embedder'])
    ...
    def __call__(self, train, dev, test, unlabel, addn, addn_un, addn_test, ext_train, ext_test, mode="train", checkPth=None):
...
#             self._evaluate(test, addn_test, model)
    ...
    def _run_epoch(self, train_data, dev_data, unlabel_data, addn_data, addn_data_unlab, addn_dev, ext_train, ext_test):
        addn_dev.cuda()
...
#                 self.embedder.train()
    ...
    def _predict_batch(self, cm, batch, addn_batch, ext_batch):
#         self.embedder.eval()
        self.encoder.eval()
        self.clf.eval()
        one, two = self.encoder(self.embedder(batch), batch)
    ...
    def _run_evaluate(self, test_data, addn_test, ext_test):
        pr_curve_data = []
        cm = ConfusionMatrix(self.classes)
        accuracy_list = []
...
#                 continue
    ...
    def _train(self, train_data, dev_data, unlabel_data, addn_data, addn_data_unlab, addn_dev, ext_train, ext_test):
    ...
    def _evaluate(self, test_data, addn_test, ext_test, mode="train"):
